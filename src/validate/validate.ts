...
export function validatePolicySyntax(
  policyDocument: any,
  ...
  allErrors.push(
    ...validateTypeOrArrayOfTypeIfExists(policyDocument.Statement, 'Statement', ['object'])
  )
  if (typeof policyDocument.Statement === 'object' && !Array.isArray(policyDocument.Statement)) {
    ...
    for (let i = 0; i < policyDocument.Statement.length; i++) {
      allErrors.push(
        ...validateStatement(policyDocument.Statement[i], `Statement[${i}]`, validationCallbacks)
      )
    }
  }
...
function validateStatement(
  ...
  statementErrors.push(
    ...validateTypeOrArrayOfTypeIfExists(statement.Action, `${path}.Action`, 'string')
  )
  statementErrors.push(
    ...validateTypeOrArrayOfTypeIfExists(statement.NotAction, `${path}.NotAction`, 'string')
  )
  ...
  statementErrors.push(
    ...validateStringOrArrayStringCallback(
      statement,
      'Action',
      path,
      validationCallbacks.validateAction
    )
  )
  statementErrors.push(
    ...validateStringOrArrayStringCallback(
      ...
      'NotAction',
      path,
      validationCallbacks.validateNotAction
    )
  )
  ...
  statementErrors.push(
    ...validateDataTypeIfExists(statement.Principal, `${path}.Principal`, ['string', 'object'])
  )
  statementErrors.push(
    ...validateDataTypeIfExists(statement.NotPrincipal, `${path}.NotPrincipal`, [
      'string',
      'object'
    ])
  )
...
function validatePrincipal(principal: any, path: string): ValidationError[] {
  ...
  if (typeof principal === 'object') {
    ...
    principalErrors.push(
      ...validateTypeOrArrayOfTypeIfExists(principal.AWS, `${path}.AWS`, 'string')
    )
    principalErrors.push(
      ...validateTypeOrArrayOfTypeIfExists(principal.Service, `${path}.Service`, 'string')
    )
    principalErrors.push(
      ...validateTypeOrArrayOfTypeIfExists(principal.Federated, `${path}.Federated`, 'string')
    )
    principalErrors.push(
...
function validateActionString(string: string, path: string): ValidationError[] {
  ...
  if (action.length > 0 && !actionRegex.test(action)) {
    errors.push({
      path,
      message: `Action can only contain letters, numbers, asterisks, question marks, commas, hyphens, and spaces`
    })
  }
...
function validateCondition(condition: any, path: string): ValidationError[] {
  ...
  for (const operator of conditionOperators) {
    ...
    conditionErrors.push(
      ...validateDataTypeIfExists(condition[operator], `${path}.${operator}`, 'object')
    )
...
function validateTypeOrArrayOfTypeIfExists(
  value: any,
  path: string,
  allowedTypes: PolicyDataType | PolicyDataType[]
): ValidationError[] {
  if (value === undefined) {
    return []
  }
...
function validateDataTypeIfExists(
  value: any,
  path: string,
  allowedDataTypes: PolicyDataType | PolicyDataType[]
): ValidationError[] {
  if (value === undefined) {
    return []
  }
...
function validateStringOrArrayStringCallback(
  ...
  fieldName: string,
  path: string,
  callback?: (value: string, path: string) => ValidationError[]
): ValidationError[] {
